    ...
    board.forEach(function (row, rowIdx) {
        row.forEach(function (cell, colIdx) {
            if (!cell.revealed && !cell.flagged) {
                var adjacentCells = getAdjacentCells(rowIdx, colIdx);
                var knownBombs = adjacentCells.filter(c => c.revealed && c.bomb).length;
                var unknownCells = adjacentCells.filter(c => !c.revealed && !c.flagged).length;
                var totalHints = adjacentCells.reduce((sum, c) => sum + (c.revealed ? c.adjBombs : 0), 0);

                // Calculate the probability of the current cell being a bomb
                var probability = unknownCells > 0 ? (totalHints - knownBombs) / unknownCells : 0;
...
function getAdjacentCells(rowIdx, colIdx) {
    var adjacentCells = [];
    for (var r = rowIdx - 1; r <= rowIdx + 1; r++) {
        for (var c = colIdx - 1; c <= colIdx + 1; c++) {
            if (r >= 0 && r < size && c >= 0 && c < size && !(r === rowIdx && c === colIdx)) {
                adjacentCells.push(board[r][c]);
            }
        }
    }
...
function aiPlay() {
    if (!hitBomb && !winner) {
        ...
        if (probabilities.length > 0) {
            // Sort by lowest probability first
            probabilities.sort((a, b) => a.probability - b.probability);
            var safestCell = probabilities[0].cell;

            // If the safest cell has a bomb, flag it; otherwise, reveal it
            if (safestCell.bomb) {
                safestCell.flag();
                var td = document.querySelector(`[data-row="${safestCell.row}"][data-col="${safestCell.col}"]`);
                td.innerHTML = flagImage;
                bombCount -= 1;  // Adjust bomb count
            } else {
                hitBomb = safestCell.reveal();
                var td = document.querySelector(`[data-row="${safestCell.row}"][data-col="${safestCell.col}"]`);
        ...
        setTimeout(aiPlay, 1000);  // Delay next move for visibility (1000ms)
    }
}

/* ----- AI Controller Button ----- */
